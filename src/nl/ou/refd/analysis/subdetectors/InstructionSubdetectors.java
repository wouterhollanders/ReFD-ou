...
public final class InstructionSubdetectors {
	...
	public static class ParentMethods extends Subdetector {
		...
		public Set<ProgramLocation> applyOn(Set<ProgramLocation> locations) {
			return new ProgramComponentsGenerator().stream().classes().methods()
					.containInstruction(new InstructionSet(locations).stream()).collect().locations();
		}
	}

	/**
	 * Integrates incoming streams of instructions unionWith into the provided set
	 * of instructions, removing duplicates.
	 * 
	...
	/**
	 * Filters the provided set of instruction locations and keeps only instructions
	 * also contained within incoming streams intersectWith.
	 * 
	...
	public static class IntersectionWithInstructions extends Subdetector {
		...
		@Override
		public Set<ProgramLocation> applyOn(Set<ProgramLocation> locations) {
			return Graph.query(locations).contained().intersection(streamVarArgToGraphQueryArray(intersectWith))
					.locations();
		}
	}

	/**
	 * Filters the provided set of instruction locations and keeps only the
	 * instructions representing calls to fields.
	 * 
	...
	public static class FieldCalls extends Subdetector {
		...
		@Override
		public Set<ProgramLocation> applyOn(Set<ProgramLocation> locations) {
			return new ProgramComponentsGenerator().stream().classes().fields().fieldsCalledAt()
	...
	/**
	 * Filters the set of instruction locations and keeps only the instructions
	 * representing calls to methods.
	 * 
	...
	public static class MethodCalls extends Subdetector {
		...
		@Override
		public Set<ProgramLocation> applyOn(Set<ProgramLocation> locations) {
			return new ProgramComponentsGenerator().stream().classes().methods().methodsCalledAt()
					.intersectionWithInstructions(
							new InstructionSet(Graph.query(locations).contained().locations()).stream())
					.collect().locations();
		}
	...
	public static class MethodAssignments extends Subdetector {
		/**
		 * {@inheritDoc}
		 */
		@Override
		public Set<ProgramLocation> applyOn(Set<ProgramLocation> locations) {
			return Graph.query(locations).contained().locationsTaggedWithAll(Tags.ProgramLocation.ASSIGNMENT)
					.locations();
		}
	}
}
