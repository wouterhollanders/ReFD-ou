 ...
 */
public class AddInstruction extends Microstep {
	
	private final InstructionSpecification instructionToAdd;
	
	/**
	 * Creates the microstep with specification of the method to add.
	 * @param classToAdd the specification of the method to add
	 */
	public AddInstruction(InstructionSpecification instructionToAdd) {
		this.instructionToAdd = instructionToAdd;
...
//		potentialRisk(new OverloadParameterConversion.Method(methodToAdd));
	...
	public void executeOnGraph(Graph graph) {
		this.instructionToAdd.construct(graph);
	}
